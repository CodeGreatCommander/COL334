...
void *get_data(void *args)
{
    ...
    while (eof)
    {
        vector<string> response;
        if (rogue)
            response = rogue_read(client_fd, count);
        else
            response = read(client_fd, count);
        if (response.empty())
        {
            continue;
        }
    ...
    std::lock_guard<std::mutex> lock(fairness_mutex);
    fairness[(int)client_id] = elapsed.count();
    ...
    else
    {
        ...
        for (size_t j = 0; j < 4; j++)
        {
            for (string &protocol : protocols)
            {
                ...
                size_t noc = 10;
                json_data["num_clients"] = noc;
                cout << "\rProtocol: " << protocol << " " << j << "/4" << flush;
                ...
                if (protocol == "fifo")
                    fi += sum * sum / (10 * sum_sq);
                else
                    rr += sum * sum / (10 * sum_sq);
