...
class CustomTopo(Topo):
    def build(self, loss, delay):
        ...
        self.addLink(h1, s1, loss=loss, delay=f'{delay}ms', max_queue_size=100)
...
def run(expname):
    ...
    for LOSS in loss_list:
        for DELAY in delay_list:
            for FAST_RECOVERY in [False, True]:
                for i in range(0, NUM_ITERATIONS):
                    ...
                    h1.cmd(f"python3 p1_server.py {SERVER_IP} {SERVER_PORT} True &")
                    result = h2.cmd(f"python3 p1_client.py {SERVER_IP} {SERVER_PORT} ")
                    # h1.cmd(f"python3 p1_server.py {SERVER_IP} {SERVER_PORT} >>{server_log} True 2>&1 &")
                    # result = h2.cmd(f"python3 p1_client.py {SERVER_IP} {SERVER_PORT} >>{client_log} 2>&1")
                    # h1.cmd(f"python3 p1_server.py {SERVER_IP} {SERVER_PORT} {FAST_RECOVERY} >/dev/null 2>&1 2>&1 &")
                    # result = h2.cmd(f"python3 p1_client.py {SERVER_IP} {SERVER_PORT} >/dev/null 2>&1 2>&1")
                    end_time = time.time()
