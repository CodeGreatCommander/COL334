...
def run():
    ...
    for DELAY in delay_list:
        for i in range(0, NUM_ITERATIONS):
            ...
            # s2_cmd = f"python3 p2_server.py {SERVER_IP2} {SERVER_PORT2} >>{server_log}_2.txt 2>&1  &"
            # c1_cmd = f"python3 p2_client.py {SERVER_IP1} {SERVER_PORT1} --pref_outfile {pref_c1} >>{client_log}_1.txt 2>&1 &"
            # c2_cmd = f"python3 p2_client.py {SERVER_IP2} {SERVER_PORT2} --pref_outfile {pref_c2} >>{client_log}_2.txt 2>&1 &"

            s1_cmd = f"python3 p2_server.py {SERVER_IP1} {SERVER_PORT1} >/dev/null 2>&1   &"
            s2_cmd = f"python3 p2_server.py {SERVER_IP2} {SERVER_PORT2} >/dev/null 2>&1  &"
            c1_cmd = f"python3 p2_client.py {SERVER_IP1} {SERVER_PORT1} --pref_outfile {pref_c1} >/dev/null 2>&1 &"
            c2_cmd = f"python3 p2_client.py {SERVER_IP2} {SERVER_PORT2} --pref_outfile {pref_c2} >/dev/null 2>&1 &"
            ...
            while len(c2_pid_raw.split()) == 0:
                c2_pid_raw = c2.cmd(f"pgrep -f 'p2_client.py {SERVER_IP2} {SERVER_PORT2}'").strip()  # Replace <process_name> with a unique identifier
                time.sleep(0.1)
            c2_pid = c2_pid_raw.split()[-1]
            print(f"started client 2 with PID: {c2_pid}")
